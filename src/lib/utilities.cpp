...
namespace hope {
  ...
  void Utilities::calRegionGrowing(const Cloud_XYZRGBN::Ptr& cloud_in, int minsz, int maxsz, int nb, int smooth,
  ...
  void Utilities::estimateNormals(const Cloud_XYZN::Ptr &cloud_in, Cloud_XYZN::Ptr &cloud_out, float dsp_th) {
    pcl::NormalEstimationOMP<Point_N, Point_N> nest;
  ...
  template <typename T>
  void Utilities::downSampling(T cloud_in, T &cloud_out, float th_xy, float th_z)
  {
    cloud_in->is_dense = false;
    std::vector<int> ind;
    pcl::removeNaNFromPointCloud(*cloud_in, *cloud_in, ind);

    if (th_xy > 0 && th_z > 0) {
      if (std::is_same<T, Cloud_XYZ::Ptr>::value) {
        pcl::VoxelGrid<pcl::PointXYZ> vg;
        vg.setInputCloud(boost::dynamic_pointer_cast<Cloud_XYZ>(cloud_in));
        vg.setLeafSize(th_xy, th_xy, th_z);
        vg.filter(*boost::dynamic_pointer_cast<Cloud_XYZ>(cloud_out));
      } else if (std::is_same<T, Cloud_XYZRGB::Ptr>::value) {
        pcl::VoxelGrid<pcl::PointXYZRGB> vg;
        vg.setInputCloud(boost::dynamic_pointer_cast<Cloud_XYZRGB>(cloud_in));
        vg.setLeafSize(th_xy, th_xy, th_z);
        vg.filter(*boost::dynamic_pointer_cast<Cloud_XYZRGB>(cloud_out));
      } else if (std::is_same<T, Cloud_XYZN::Ptr>::value) {
        pcl::VoxelGrid<pcl::PointNormal> vg;
        vg.setInputCloud(boost::dynamic_pointer_cast<Cloud_XYZN>(cloud_in));
        vg.setLeafSize(th_xy, th_xy, th_z);
        vg.filter(*boost::dynamic_pointer_cast<Cloud_XYZN>(cloud_out));
      } else {
        throw std::runtime_error("Not supported cloud type");
      }
    } else {
      cloud_out = cloud_in;
    }
...
//template<typename T>
  ...
  bool Utilities::alignmentWithFPFH(Cloud_XYZN::Ptr src_cloud, PointCloudFPFH::Ptr src_features,
    ...
    pcl::SampleConsensusPrerejective<Point_N, Point_N, FeatureFPFH> align;
  ...
  void Utilities::combineCloud(const Cloud_XYZ::Ptr& cloud_a, const Cloud_XYZ::Ptr& cloud_b, Cloud_XYZ::Ptr &cloud_out) {
...
// refer: https://raymii.org/s/snippets/Cpp_template_definitions_in_a_cpp_file_instead_of_header.html
  ...
  template void Utilities::downSampling<Cloud_XYZ::Ptr>(Cloud_XYZ::Ptr cloud_in, Cloud_XYZ::Ptr &cloud_out, float th_xy, float th_z);
  template void Utilities::downSampling<Cloud_XYZRGB::Ptr>(Cloud_XYZRGB::Ptr cloud_in, Cloud_XYZRGB::Ptr &cloud_out, float th_xy, float th_z);
  template void Utilities::downSampling<Cloud_XYZN::Ptr>(Cloud_XYZN::Ptr cloud_in, Cloud_XYZN::Ptr &cloud_out, float th_xy, float th_z);
